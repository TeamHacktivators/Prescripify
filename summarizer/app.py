...
def extract_prescription(llm, cleaned_text):
    prompt_extract = PromptTemplate.from_template(
        ...
        Your job is to extract the medicines and return them in JSON format containing the following keys: `patient` storing the patient name, `medicine` containing `name`, `dosage`, `duration` and `description` where name is the name of the medicine provided, dosage is the times the medicine should be consumed in a day, duration is the no of days the medicine should be consumed, description is a one liner description of the medicine  and a field `tips` contains addition information provided by the doctor containing prohibited items and recommended items.
